...
import {
  ArrowDown, ArrowRight, ArrowUp, Bell, Bot, Check, ChevronDown, CircleUserRound, BookOpenCheck, Building2,
...
function Counselling() {
  ...
  return <div className="page">
    ...
    <section className="preference-builder"><div className="section-heading"><div><span className="eyebrow">PREFERENCE SEQUENCE</span><h2>Your working order</h2></div><span className="updated">Changes saved locally</span></div><div className="preference-list">{order.map((college, i) => <div className="preference-row" key={college.id}><span className="drag">::</span><strong className="pref-number">{String(i+1).padStart(2,'0')}</strong><div className="college-monogram">{college.short.slice(0,2)}</div><div className="pref-name"><strong>{college.name}</strong><span>{college.branch} - {college.category}</span></div><div className="prob"><span>Admission</span><strong>{college.probability}%</strong></div><div className="move-buttons"><button onClick={() => move(i,-1)} disabled={i===0} aria-label={`Move ${college.short} up`} title="Move up"><ArrowUp size={17}/></button><button onClick={() => move(i,1)} disabled={i===order.length-1} aria-label={`Move ${college.short} down`} title="Move down"><ArrowDown size={17}/></button></div></div>)}</div></section>
